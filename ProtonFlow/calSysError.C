...
static const TString ParType[2] = {"proton","antiproton"};
...
static const Int_t Sys_total = 18;
...
void calSysError(Int_t mEnergy = 0, Int_t mCharge = 0, Int_t mOrder = 1)
{
  TString InPutFile = Form("./OutPut/AuAu%s/Proton/flow_%s.root",Energy[mEnergy].Data(),Order[mOrder].Data());
  ...
  for(Int_t i_cent = Cent_start; i_cent < Cent_stop; i_cent++) // centrality loop
  {
	...
	{
	  TString KEY_Flow = Form("Flow_%s_Proton_Centrality_%d_Charge_%d_EtaGap_%d_SysError_%d",Order[mOrder].Data(),i_cent,i_charge,i_eta,i_sys);
  ...
  TString KEY_Default = Form("Flow_%s_Proton_Centrality_0_Charge_%d_EtaGap_0_SysError_0",Order[mOrder].Data(),mCharge);
  ...
  TString OutPutFile = Form("./OutPut/AuAu%s/Proton/TriFlow_%s_SysError.root",Energy[mEnergy].Data(),ParType[mCharge].Data());
  cout << "OutPutFile set to: " << OutPutFile.Data() << endl;
  TFile *File_OutPut = new TFile(OutPutFile.Data(),"RECREATE");
  File_OutPut->cd();
  h_frame->Write();
  TString StatErrorV3 = Form("g_TriFlow_%s_%s_StatError",Energy[mEnergy].Data(),ParType[mCharge].Data());
  g_mFlow[KEY_Default]->SetName(StatErrorV3.Data());
  g_mFlow[KEY_Default]->Write();
  TString SysErrorV3 = Form("g_TriFlow_%s_%s_SysError",Energy[mEnergy].Data(),ParType[mCharge].Data());
